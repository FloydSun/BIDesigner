...
var SaveQueryEx = Modal.extend({
    ...
    initialize: function(args) {
        ...
        this.message = _.template(
            "<form id='save_query_form'>" +
            "<label for='name' class='i18n'>名称:</label>&nbsp;" +
    ...
    save: function(event) {
        ...
        var foldername = this.folder_name; /* XXX == root, should it be something different than ''? */
        /*
        ...
        if (this.folder_name !== null && this.folder_name !== undefined && this.folder_name.length > 0) {
            if (name !== null && name.length > 0) {
    			this.repository.fetch({success: function(collection, response){
    				...
    				if(paths.indexOf(foldername + name + ".saiku")> -1 && self.query.get("name") != (name)){
    					new OverwriteModalCN({name: name, foldername: foldername, parent: self}).render().open();
						$( self.el ).prev().children('span').text('保存查询');
    				}
    				else{
